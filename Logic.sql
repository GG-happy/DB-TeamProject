...
FOR EACH ROW
BEGIN
    InsertAppliedRate(:NEW.evaluation_id);
END;
/
*/
...
BEGIN
    ...
    -- 신용 점수 계산
    C_SCORE := (F_SCORE + (R_SCORE * 10)) / 2;
    
    -- 신용 점수에 따른 한도 측정
    IF C_SCORE >= 900 THEN LOAN := 10000000000;    -- 100억
    ELSIF C_SCORE >= 800 THEN LOAN := 5000000000;  -- 50억
    ELSIF C_SCORE >= 700 THEN LOAN := 1000000000;  -- 10억
    ELSIF C_SCORE >= 600 THEN LOAN := 500000000;   -- 5억
    ELSIF C_SCORE >= 500 THEN LOAN := 100000000;   -- 1억
    ELSIF C_SCORE >= 400 THEN LOAN := 50000000;    -- 5천
    ELSIF C_SCORE >= 300 THEN LOAN := 10000000;    -- 1천
    ELSIF C_SCORE >= 200 THEN LOAN := 5000000;     -- 5백
    ELSIF C_SCORE >= 100 THEN LOAN := 1000000;     -- 1백
    ELSE LOAN := 0;
    END IF;

    -- 신용평가 삽입
    INSERT INTO CreditEvaluation (evaluation_date, credit_score, loan_limit, company_id)
    VALUES ( SYSDATE, C_SCORE, LOAN, COM_ID); -- 평가 날짜, 신용 점수, 초기 대출 한도, 기업 ID
...
/*
CREATE OR REPLACE TRIGGER Update_FF2CS_TRG  --> 트리거 충돌 발생으로 트랜잭션으로 구현
...
/*
CREATE OR REPLACE TRIGGER Update_RE2CS_TRG  --> 트리거 충돌 발생으로 트랜잭션으로 구현
...
-- 신용 점수에 따른 신용 한도 변경
-------------------------------------
--CREATE OR REPLACE PROCEDURE UpdateLimit(EVA_ID IN VARCHAR2) AS  --> UPDATECREDITSCORE프로시저와 합침
--    SCORE NUMBER := 0;  -- 신용 점수 초기화
--    LOAN NUMBER := 0;   -- 대출 한도 초기화
--BEGIN
--    -- 최신 신용 점수 가져오기
